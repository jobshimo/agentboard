# Daemon Capability Specification
...
Cross-references: `launcher.md` (subcommands, idempotent spawn), `storage.md` (per-repo DB cache, registry), `mcp-surface.md` (inter-process notify), `realtime-ui.md` (daemon endpoints, broadcast).

---

## Requirements

### Requirement: Daemon Identity and Port
...
1. On startup:
   a. Probe port (see `launcher.md` idempotent spawn protocol).
...
2. During operation:
   ...
   d. Maintain the per-repo DB cache (see `storage.md`).
   e. Update and debounce-flush `daemon.json` on new repo access.
...
3. On shutdown (SIGTERM or SIGINT):
   ...
   c. Close all `Database` instances in the cache.
...
- AND `POST /internal/notify` receives `{ "repo": "C:\\Projects\\Foo" }`
...
### Requirement: Failure Modes and Recovery

| Failure | Expected Behavior |
|---------|-------------------|
| Port 7733 occupied by agentboard | Skip bind, open browser (see `launcher.md`) |
